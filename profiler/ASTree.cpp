...
void AST::fileHeader(const std::string& profileName) {
    ...
    line = "#include \"profile.hpp\" \n";
    child.insert(tmp, new AST(token,line));
}
...
AST* temp = nullptr;
std::list<AST*>::iterator itr2;
    while(itr != child.end()){
        if((*itr)->tag == "function" ||(*itr)->tag == "constructor" || (*itr)->tag == "destructor")
        {
            temp = (*itr)->getChild("name");
            functionName = (temp)->getName();
            itr2 = (*itr)->child.begin();
            while((*itr2)->tag != "block")
            {
                ++itr2;
            }
            itr2 = (*itr2)->child.begin();


            insertLine = " " + profileName + " .count(__LINE__," + "\"" + functionName + "\");";
            child.insert(++itr2,new AST(token,insertLine));
        }
        ++itr;
    }
}
...
void AST::lineCount(const std::string& profileName) {
    ...
    for(auto itr = child.begin(); itr != child.end(); ++itr)
    {
        if((*itr)->nodeType == category)
        {
            ...
            {
                child.insert(itr,new AST(token, insertLine));
            }else if(!isStopTag((*itr)->tag)){
